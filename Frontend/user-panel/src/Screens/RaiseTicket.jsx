...
function RaiseTicket() {
    ...
    const fetchProducts = async () => {
      try {
        ...
        const response = await fetch(
          createUrl(`home/History/Purchases/user/${userId}`)
        );
        const data = await response.json();
        if (data && Array.isArray(data)) {
          const orderedProducts = data
            .filter(
              (order) => order.product_id !== null && order.productName !== null
            ) // Filter out products with null id or name
            .map((order) => ({
              id: order.product_id,
  ...
  const handleOnSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(createUrl("ticket/newticket"), {
        method: "POST",
        headers: {
